    ...
    return function (WrappedComponent) {
      var Form = function (_Component) {
        ...
        _createClass(Form, [{
          ...
          value: function validateFieldsErrors(values) {
            var registeredFields = this.props.registeredFields;

            return registeredFields.reduce(function (errObj, field) {
              var validate = getIn(field, 'validate');
              if (typeof validate === 'function') {
                var name = getIn(field, 'name');
                var error = validate(values[name]);
            ...
            if (validate) {
              if (nextProps) {
                ...
                if (!deepEqual(values, nextProps.values)) {
                  ...
                  var fieldNoErrors = this.validateFieldsErrors(nextProps.values);
                  this.updateSyncErrorsIfNeeded(_extends({}, nextSyncErrors, fieldNoErrors));
                }
              } else {
                // initial render
                var _nextSyncErrors = validate(values, this.props);
                var _fieldNoErrors = this.validateFieldsErrors(values);
                this.updateSyncErrorsIfNeeded(_extends({}, _nextSyncErrors, _fieldNoErrors));
              }
            }
          }
        }, {
          ...
          key: 'register',
          value: function register(name, type, validate) {
